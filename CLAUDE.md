...
The app itself is unindexable (Keycloak `login-required` sends a crawler straight to the login page), so `/` serves one of two pages by the `wac_app` cookie (`wac/frontend/nginx.conf`, `map $cookie_wac_app`): no cookie → the static landing page (crawlers and new visitors), cookie → the Angular app (`X-Robots-Tag: noindex`). The landing's "Accedi" button goes to `/enter`, which sets the cookie (`Max-Age` 1 year, `Secure; HttpOnly; SameSite=Lax`) and 302s to `/`; the PWA manifest's `start_url` is `/enter` (with `id: "/"` to keep installed apps' identity) so installed apps skip the landing. The choice depends on user state, not user-agent, so it is not cloaking; `/` sends `Vary: Cookie` + `Cache-Control: no-cache` — **a cache in front of nginx (Cloudflare) must not cache the HTML of `/`**. Keycloak, `sw.js` and Angular routing are untouched.

`wac/frontend/landing/` is a dependency-free static generator: `template.html` + `landing.css` + `i18n/<lang>.json` → `build.mjs` renders `/` (Italian), `/en/`, `/fr/`, `/de/`, `/es/` (linked by hreflang/`x-default`, no `Accept-Language` redirect) and `sitemap.xml` into `dist/…/browser`. It runs as the npm `postbuild` hook, so `npm run build` (CI and the Dockerfile) always produces it, and it fails the build on unresolved `{{placeholders}}`, title >60 / description outside 70–160 chars, ≠1 `<h1>`, incomplete hreflang or a missing `og-image.png`. The landing dictionaries are a fourth `i18n.mjs` bundle (`npm run i18n:translate -- --bundle=landing`, Italian hand-edited; check the generated `meta.description` lengths and brand name after translating — DeepL has mangled "WacChat"). No JS on the landing (the CSP is `default-src 'self'`, inline scripts are blocked; JSON-LD is data and unaffected), and the CSS is emitted as `landing-<hash>.css` for the same Cloudflare-ignores-query-strings reason as `login-vN.css`. `/landing/**` is 404 (the Italian page's only URL is `/`); `public/robots.txt` disallows `/enter`, `/api/`, `/ws`. Security headers live in `security-headers.inc`, `include`d in every nginx `location` that calls `add_header` (a location's own `add_header` cancels the server-level ones).
